...
def drawBboxAndText(frame, box, text, min_dist):
    ...
    font = ImageFont.truetype("font/NanumGothicExtraBold.ttf", int(bbox_width * 0.06))
    ...
    # frame decorate 
    frame = cv2.rectangle(frame, (resized_bbox_pt1_x, int(resized_bbox_pt1_y - (resized_bbox_pt2_y - resized_bbox_pt1_y) * 0.06)) , (resized_bbox_pt2_x ,resized_bbox_pt1_y ), (0,0,255), -1)
    img = Image.fromarray(frame)
    draw = ImageDraw.Draw(img)
    draw.text((int(resized_bbox_pt1_x + resized_bbox_width * 0.05), int(resized_bbox_pt1_y - resized_bbox_height * 0.05)), text, font = font, fill = (0, 255, 0))
        ...
        if face['det_score'] > 0.5:
          ...
          if text == '미등록':
            ...
            if len(idx_list[0]) == 0:
              ...
              if new_min_dist > 0.35: # 새로운 Unknown이면 db에 등록
          ...
          #   threading.Thread(target = playTTSandUpdateTimeTerm, args = (timeDict, nametts)).start()
